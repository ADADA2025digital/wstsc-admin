...
export const EnrolmentFormProvider = ({ children }) => {
  ...
  const getValidationRules = (section, field) => {
    const optionalFields = {
      ...
      parent_carer_2: [
        "title", "gender", "relationship_to_student", "first_name", "last_name", "middle_name",
        "country_of_birth", "date_of_birth", "nationality", "email", "mobile_phone", "alternative_phone",
        "marital_status", "occupation", "street_number", "street_name", "suburb", "state", "postal_code",
      ...
      second_emergency_contact: [
        "family_name", "given_name", "relationship_to_student", "mobile_phone", "home_phone", "work_phone",
      ],
      medical_details: [
        "asthma", "major_illness", "allergies", "special_learning_needs", "special_learning_needs_details",
      ],
  ...
  const validateField = (section, field, value) => {
    ...
    // Required field validation
      ...
      if (field === "attendance_to" && value && formData.student.attendance_from) {
  ...
  const validateForm = (personalDeclarationOnly = false) => {
    ...
    if (personalDeclarationOnly) {
      ...
      const sections = [
        ...
        "first_contact", "second_contact", "parent_not_living", "first_emergency_contact",
    ...
    const sanitizeStrings = (obj, path = '') => {
      ...
      Object.keys(obj).forEach(key => {
        ...
        if (typeof obj[key] === 'string') {
          ...
          } else if (fullPath.includes('name') || fullPath.includes('given_name') || fullPath.includes('family_name')) {
            obj[key] = obj[key].substring(0, 30);
          } else if (fullPath.includes('school_name') || fullPath.includes('occupation') || fullPath.includes('street_name')) {
          ...
          if (original !== obj[key]) {
            console.log(`✂️  ${fullPath}: Trimmed from "${original}" to "${obj[key]}"`);
          }
        } else if (typeof obj[key] === 'object' && obj[key] !== null && !(obj[key] instanceof Date) && !Array.isArray(obj[key])) {
    ...
    try {
      ...
      const submissionData = {
        student: {
          ...
          first_given_name: formData.student.first_given_name?.trim().substring(0, 30) || "",
          ...
          community_school_name: formData.student.community_school_name?.trim().substring(0, 50) || "",
          day_school_name: formData.student.day_school_name?.trim().substring(0, 50) || "",
        },
        parent_carer_1: formData.parent_carer_1.first_name ? {
          ...
          relationship_to_student: formData.parent_carer_1.relationship_to_student || "",
          first_name: formData.parent_carer_1.first_name?.trim().substring(0, 30) || "",
          last_name: formData.parent_carer_1.last_name?.trim().substring(0, 30) || "",
          middle_name: formData.parent_carer_1.middle_name?.trim().substring(0, 30) || "",
          country_of_birth: formData.parent_carer_1.country_of_birth?.trim().substring(0, 30) || "",
          ...
          street_name: formData.parent_carer_1.street_name?.trim().substring(0, 50) || "",
        ...
        parent_carer_2: formData.parent_carer_2.first_name ? {
          ...
          relationship_to_student: formData.parent_carer_2.relationship_to_student || "",
          first_name: formData.parent_carer_2.first_name?.trim().substring(0, 30) || "",
          last_name: formData.parent_carer_2.last_name?.trim().substring(0, 30) || "",
          middle_name: formData.parent_carer_2.middle_name?.trim().substring(0, 30) || "",
          country_of_birth: formData.parent_carer_2.country_of_birth?.trim().substring(0, 30) || "",
          ...
          street_name: formData.parent_carer_2.street_name?.trim().substring(0, 50) || "",
        ...
        personal_declaration: {
          ...
          first_parent_carer_name_date: formData.personal_declaration.first_parent_carer_name_date || "",
          second_parent_carer_name: formData.personal_declaration.second_parent_carer_name?.trim().substring(0, 60) || "",
          second_parent_carer_name_date: formData.personal_declaration.second_parent_carer_name_date || "",
          photo_video_consent: Boolean(formData.personal_declaration.photo_video_consent),
          medical_treatment_consent: Boolean(formData.personal_declaration.medical_treatment_consent),
        },
        medical_details: {
          ...
          special_learning_needs: formData.medical_details.special_learning_needs || "",
          special_learning_needs_details: formData.medical_details.special_learning_needs_details || "",
        },
      };
      ...
      // Set both success and submission result - DO NOT RESET FORM HERE
      setSuccess(true);
  ...
  const submitTestData = async () => {
    ...
    try {
      ...
      console.error("❌ Test submission failed:", err.response?.data || err.message);
      throw err;
    }
  };
